  ...
  <div v-for="(typeface, key) in datas">
    ...
    <div class="sg-MainContent-flexbox">
        <div class="sg-ColorCard" v-for="(nameFont, keyFont) in typeface.fonts" >
          <p style="font-size:4em;text-align:center;color:black;opacity:1; " v-bind:style="{'font-family': key +'-'+ keyFont}" >
            Aa
          </p>
    ...
    <div class="sg-MainContent-flexbox" >
        <div class="sg-Card sg-TypescaleCard" v-for="(params, breakpoint) in typeface.responsive">
          ...
          <p class="daf-warning">{{ params.typescale }} / !\MISSING<span>Typescale</span></p><br/>
        ...
        <tr v-for="(fontSizeDatas, fontSizeNamespace) in typeface.responsive.mobile['font-sizes']" :class="[(fontSizeNamespace == 'root') ? 'root': '']" >
          <th>{{ fontSizeNamespace }}</th>
            <template v-for="(params, breakpoint) in typeface.responsive">
              <td v-if="params.multiplier != 1">
                <p class="daf-warning"> / !\MISSING</p>
              <span>{{ typeface.responsive[breakpoint]['font-sizes'][fontSizeNamespace].min }}</span>
              <span>{{ typeface.responsive[breakpoint]['font-sizes'][fontSizeNamespace].max }}</span>
              </td>
              <td v-else>
                <p class="daf-warning"> / !\MISSING</p>
                <span>{{ typeface.responsive[breakpoint]['font-sizes'][fontSizeNamespace].min }}</span>
    ...
    <div v-for="(nameFont, keyFont) in typeface.fonts">
      ...
      <div class="sg-Table-Container">
            <div class="sg-TypescaleDesktop" v-for="(params, name) in typeface.responsive.mobile['font-sizes'] ">
              <p v-bind:style="{'font-family': key +'-'+ keyFont}" > <!-- style="font-size:{{ params['font-size'] }}; " -->
                The quick brown fox jumps over the lazy dog
              </p><p class="daf-warning"> / !\MISSING</p>
...
<style>
 ...
 .sg-MainContent-flexbox {
  ...
  .sg-DesignSystemItem .daf-warning {
    color: orangered;
  }
</style>